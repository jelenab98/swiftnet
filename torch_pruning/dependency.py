from models.util import (SeparableConv2d, _BNReluConv, _ConvBNReLuConv, _ConvBNReLu,
                         _Upsample, _UpsampleSE, _UpsampleBlend, _UpsampleCBRC,
                         _UpsampleShared, _UpsampleBlendBottlencek, _UpsampleNoSKip)
...
class DependencyGraph(object):
    PRUNABLE_MODULES = [nn.modules.conv._ConvNd, nn.modules.batchnorm._BatchNorm, nn.Linear, nn.PReLU,
                        SeparableConv2d, _BNReluConv, _ConvBNReLuConv, _ConvBNReLu,
                        _Upsample, _UpsampleSE, _UpsampleBlend, _UpsampleCBRC,
                        _UpsampleShared, _UpsampleBlendBottlencek, _UpsampleNoSKip]
    ...
    def build_dependency(self, model: torch.nn.Module, example_inputs: torch.Tensor, output_transform: callable = None,
                         verbose: bool = True):
        ...
        if isinstance(out, (list, tuple)):
            for o in out:
                if isinstance(o, dict):
                    for o_i in o.values():
                        if isinstance(o_i, list):
                            for o_j in o_i:
                                _build_graph(o_j.grad_fn)
                        else:
                            _build_graph(o_i.grad_fn)
                else:
