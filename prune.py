...
from models.resnet.resnet_single_scale import BasicBlock
from models.util import get_n_params


def prune_model(model, percentage=0.05, p=1, H=1024, W=2048):
    model.cpu()
    prune_stats = []
    DG = tp.DependencyGraph().build_dependency(model, torch.randn(1, 3, H, W))
    ...
    for m in model.modules():
        if isinstance(m, BasicBlock):
            prune_conv(m.conv1, percentage, p)
            prune_conv(m.conv2, percentage, p)
...
class PruningTrainer:
    def __init__(self, conf, args, name, prune_stats):
        self.conf = conf
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        model, prune_stats = prune_model(conf.model.cuda())
        conf.model = model
        print(conf.model)
        ...
        optimizer, scheduler = conf.reset_optimizer()
        conf.optimizer = optimizer
        conf.lr_scheduler = scheduler
        with PruningTrainer(conf, args, conf_path.stem, prune_stats) as trainer:
            trainer.train()
